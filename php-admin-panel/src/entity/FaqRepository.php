...
class FaqRepository
{
    ...
    /**
     * Fetches all unsynced FAQ records from the database.
     *
     * @return array An associative array of unsynced FAQ records. Returns an empty array if none are found or on failure.
     */
    ...
    /**
     * Fetches FAQ IDs and their sync status for a given category.
     *
     * @param int $categoryId The ID of the category.
     * @return string Returns an empty string or actual category name
     */
    ...
    public function createCategory(string $categoryName): bool
    {
        ...
        try {
            ...
            // If execution was successful, return true
            // Otherwise, return false
            if ($execResult) {
                return true;
            } else {
    ...
    /**
     * Deletes a specific category record by its ID and all associated FAQs.
     *
     * @param int $id The ID of the category to delete.
     *
     * @return bool Returns true if the category and associated FAQs were deleted successfully, false otherwise.
     */
    public function deleteCategory(int $id): bool
    {
        ...
        try {
            ...
            $stmtFaq = $db_conn->prepare("DELETE FROM `FAQ` WHERE `category_id` = :id");
            $stmtFaq->bindParam(':id', $id);
            $stmtFaq->execute();

            // Then delete the category
            $stmtCategory = $db_conn->prepare("DELETE FROM `Category` WHERE `id` = :id");
            $stmtCategory->bindParam(':id', $id);
            $execResult = $stmtCategory->execute();
    ...
    /**
     * Searches categories based on a search term.
     *
     * @param string $searchTerm The term to search for in category names.
     *
     * @return array An array of associative arrays that match the search criteria.
     */
    public function searchCategories(string $searchTerm): array
    {
        ...
        try {
            ...
            // SQL Statement
            $sql = "SELECT * FROM `Category` WHERE `category` LIKE :term";
            ...
            // If execution was successful, return objects
